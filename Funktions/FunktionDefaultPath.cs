...
namespace MIM_Tool.Funktions
{
    class FunktionDefaultPath
    {
            ...
            {   
                ...
                if (System.IO.Directory.Exists(resolvedPath))
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        Log.err("Dein Erstellen der Unterordner ist etwas schiefgelaufen, Schauer evtl nach der Zugriffsberechtigung", ex,true);                        // Speichert den Fehler in den Einstellungen.
        ...
        public void OrdnerAbfrage()
        {
            ...
            {
                Log.err($"Beim Erstellen des Pfades: {Properties.Settings.Default.pfadDeskOK} ist etwas schiefgelaufen",ex);                                                                    // Speichert den Fehler in den Einstellungen.
                if (Properties.Settings.Default.InizialisierungAktiv) Properties.Settings.Default.InizialisierungsFehler = true;                                                                // Setzt den Initialisierungsfehler auf true.
                Properties.Settings.Default.Save();
            }
        ...
        public void ResetPath()
        {
            ...
            {                                  
                Log.war("Zurücksetzten abgebrochen.");                                                                                                                                          // Zeigt eine Abbruchmeldung an.
            }
        }
        //-------------------------------------------------------------------------------------------------Admin Funktionen-------------------------------------------------------------------------------------------------
        public void OpenConfig()
        {
            ...
            if (configFolder != null)
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    {
                        Log.err("Das Öffnen wollte irgendwie nich",ex,true);                                                                                              // Zeigt eine Fehlermeldung an, wenn ein Fehler auftritt.
                    }
                }
                else
                {
                    Log.war("Was möchtest du nicht in die Tiefe der Skripte eintauchen.");                                                                                // Zeigt eine Abbruchmeldung an.
                }
            }
            else
            {
                Log.err("Kein gültiger Konfigurationsordner gefunden.");                                                                                                  // Zeigt eine Fehlermeldung an, wenn kein Konfigurationsordner gefunden wurde.
            }
        }
    }
}
