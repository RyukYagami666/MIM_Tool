...
namespace MIM_Tool.Funktions
{
    internal class FunktionDesktopOK
    {
        ...
        public void DODStart()                                                                                     // Download von DesktopOK.
        {
            ...
            if (Properties.Settings.Default.eDeskOkDownloadReady)
            {
                ...
                if (System.IO.File.Exists(pathFile))
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        {
                            Log.err($"Fehler beim erneuten Herunterladen der Datei", ex, true);//ex,alarm           // Speichert den Fehler in den Einstellungen und zeig ihn an
                ...
                else if (System.IO.Directory.Exists(pathFolder))
                {
                    ...
                    {
                        Log.err($"Fehler beim Herunterladen der Datei", ex, true);//ex,alarm           // Speichert den Fehler in den Einstellungen und zeig ihn an
                        if (Properties.Settings.Default.InizialisierungAktiv) Properties.Settings.Default.InizialisierungsFehler = true;                                                 // Setzt den Initialisierungsfehler auf true.
                        Properties.Settings.Default.Save();                                                                                                                              // Speichert die Einstellungen.
                    }
                }
                ...
                {
                    Log.err($"Fehler: Unterordner {pathFolder} nicht vorhanden.", null, true);//ex,alarm           // Speichert den Fehler in den Einstellungen und zeig ihn an
                    if (Properties.Settings.Default.InizialisierungAktiv) Properties.Settings.Default.InizialisierungsFehler = true;                                                 // Setzt den Initialisierungsfehler auf true.
                    Properties.Settings.Default.Save();                                                                                                                              // Speichert die Einstellungen.
                }
            }
            ...
            {
                Log.err("Fehler: eDeskOkDownloadReady ist falsch oder keine Internetverbindung.",null,true);//ex,alarm           // Speichert den Fehler in den Einstellungen und zeig ihn an
                if (Properties.Settings.Default.InizialisierungAktiv) Properties.Settings.Default.InizialisierungsFehler = true;                                                 // Setzt den Initialisierungsfehler auf true.
                Properties.Settings.Default.Save();                                                                                                                              // Speichert die Einstellungen.
            }
        }
        ...
        public void IconSavePos()                                                                                      // Mit DesktopOK Positionen speichern.
        {
            ...
            if (Properties.Settings.Default.eDeskOkSavePosReady)
            {
                ...
                Thread.Sleep(2000); // Wartet zwei Sekunden.                                                                                  // Wartet zwei Sekunden.
            }
        }
        ...
        public static void IconRestore(string pathExe, string pathLastData)             // Mit DesktopOK Positionen der Icons wiederherstellen.
        {
            ...
            if (Properties.Settings.Default.eDeskOkDataReedDone && !string.IsNullOrEmpty(Properties.Settings.Default.eDeskOkLastSave))
            {
                ...
                {
                    Log.err("Fehler beim Öffnen des Dokuments.", ex, true);//ex,alarm           // Speichert den Fehler in den Einstellungen und zeig ihn an                                                                      // Speichert die Einstellungen.
                }
            }
            else
            {
                Log.err("Das Lesen der Daten ist nicht abgeschlossen oder die letzte gespeicherte Datei ist nicht vorhanden.");//ex,alarm           // Speichert den Fehler in den Einstellungen und zeig i
            }
        }
