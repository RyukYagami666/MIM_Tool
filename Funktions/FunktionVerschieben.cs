...
namespace MIM_Tool.Funktions
{
    public class FunktionVerschieben
    {
        ...
        public void Verschieben1Control()                                                                                                 // Methode zur Steuerung des Verschiebens der Icons für Monitor 1.
        {
            ...
            {
                Log.err($"Monitor 1 vorhanden, aber Speicherpfad: {storagePath} existiert nicht.", null,true);                            // Zeigt eine Fehlermeldung an.
        ...
        public void MoveDeskToPath(int index)                                                                                                                   // Methode zum Verschieben der gespeicherten Liste vom Desktop zum Speicher.
        {
            ...
            bool[] statusMonitorIconsVerstaut =
            {
            ...
            if (!statusMonitorIconsVerstaut[index] && !string.IsNullOrEmpty(zugewieseneIcons[index]))                                                                                                      // Überprüft, ob die Icons bereits verstaut sind.
            {
            ...
            {
                Log.war("Icons schon verstaut oder lehre Liste", 2000);                                                                                                             // Zeigt eine Fehlermeldung an.
            }
        }
            ...
            bool[] statusMonitorIconsVerstaut =
           {
            ...
            if (statusMonitorIconsVerstaut[index] && !string.IsNullOrEmpty(zugewieseneIcons[index]))                                                                                                      // Überprüft, ob die Icons bereits verstaut sind.
            {
                ...
                foreach (string icon in deskIconPath)
                {
                    ...
                    {
                        Log.war($"{System.IO.Path.GetFileName(icon)} schon verschoben.", 4000);                  // Zeigt eine Fehlermeldung an.
                    }
                    else
                    {
                        Log.err($"{System.IO.Path.GetFileName(icon)} nicht mehr vorhanden.\nIcons bitte neu zuweisen.", null, true);    // Zeigt eine Fehlermeldung an.
                    }
                }                                                                                                   // Ruft die Methode zum Wiederherstellen der Icons auf.
            ...
            {
                Log.war("Icons schon auf dem Desktop oder leere Liste", 2000);                                                                                           // Zeigt eine Fehlermeldung an.
            }
        }
    }
}
